/**
 * Created by MAKS on 27.09.2017.
 */
/**
 * Created by MAKS on 27.09.2017.
 */
import React, {Component} from 'react';
import Webix from './Webix';

function getForm(){
    return {
        rows: [
            { template:"Row One", height: 50},
            { cols:[
                { template:"Column One", height: 75},
                { template:"Column Two"}
            ]}
        ]
    }
}

function getTableView(){
    return {
        rows:[
            { view:"template",
                type:"header", template:"My App!" },
            { view:"datatable",
                autoConfig:true,
                height: 200,
                data:[
                    {title:"My Fair Lady", year:1964, votes:533848, rating:8.9, rank:5},
                    {title:"My Fair Lady", year:1964, votes:533848, rating:8.9, rank:5},
                    {title:"My Fair Lady", year:1964, votes:533848, rating:8.9, rank:5},
                    {title:"My Fair Lady", year:1964, votes:533848, rating:8.9, rank:5},
                ]
            }
        ]
    }
}
function getToolBar() {
    return {
        width:500,
        rows: [
            { view:"toolbar", elements:[
                { view:"button", value:"Add",    width:100 },
                { view:"button", value:"Delete", width:100 }
            ]}
        ]
    }
}
function getComplexForm(){
    return {
        width:500,
        rows: [
            getToolBar() ,
            { height:120, cols:[
                {view:"form", elements:[
                    { view:"text", placeholder:"Title"},
                    { view:"text", placeholder:"Year"}
                ]},
                {
                    view:"list",
                    template:"#title# - #year#", // which data to show
                    select:true, //enables selection
                    height:400,
                    data: [
                        { id:1, title:"The Shawshank Redemption", year:1994},
                        { id:2, title:"The Godfather", year:1972},
                        { id:3, title:"The Godfather: Part II", year:1974},
                        { id:4, title:"The Good, the Bad and the Ugly", year:1966},
                        { id:5, title:"My Fair Lady", year:1964},
                        { id:6, title:"12 Angry Men", year:1957}
                    ]
                }
            ]}
        ]
    }
}

var pivot_dataset = [
    {"name": "China", "year": 2005, "form": "Republic", "gdp": 181.357, "oil": 1.545 },
    {"name": "China", "year": 2006, "form": "Republic", "gdp": 212.507, "oil": 1.732 },
]

function getPivotTable() {
    return {
        data: pivot_dataset,
        view: "pivot",
        container: "testA",
        id: "pivot",
        max: true,
        structure: {
            s: '1',
            rows: ["form", "name"],
            columns: ["year"],
            values: [
                { name: "gdp", operation: "sum"},
                { name: "oil", operation: "sum"}
            ]
        }
    }
}
export default class WebixComponent extends Component {


    render() {
        return (

                <div>
                    <h1>Webix components</h1>

                    <Webix ui={getPivotTable()}/>

                    <Webix ui={{view:"slider"}} data={50}/>
                    <Webix ui={getForm()}/>
                    <Webix ui={getTableView()}/>
                    <Webix ui={getComplexForm()}/>

                </div>


        )
    }
}